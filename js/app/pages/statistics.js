export const statistics = {
	...
	methods:{	
		...
		get:function(){
			...
			if(this.count!="") data.append('count',this.count);
			self.loader=1;
			axios.post(this.parent.url+"/site/getStatistics?auth="+this.parent.user.auth,data).then(function(response){
				...
				//if(response.data.counts && response.data.counts[0] && !self.type) self.type = response.data.types[0].id;
				...
				<popup ref="copy" :title="'Copy banner'">
					<div class="form inner-form">
						<form v-if="parent.formData">
							...
							<div class="row">
								<label>Count of banners in column</label>
								<select v-model="count" @change="get()" required>
									<option value="0">---</option>
